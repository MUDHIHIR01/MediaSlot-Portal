...
interface DepartmentData {
  ...
  name: string;
  location: string;
  // Add other fields that might be returned by Department::all()
}
  ...
  return (
    <div className="relative">
      <div className="flex gap-2 items-center">
        <Link to={`/edit-department/${departmentId}`} className="p-1 text-blue-500 hover:text-blue-600">
...
export default function Departments() {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      setData(response.data.data); // Using .data.data to match backend response structure
      setLoading(false);
    } catch (err: any) {
  ...
  const columns = useMemo(() => [
    ...
    { Header: 'Location', accessor: 'location' },
    {
      ...
      Cell: ({ value }) => <ActionButtons departmentId={value} />,
    },
  ], []);
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Departments Data', 20, 10);
    autoTable(doc, {
      head: [['#', 'Name', 'Location']],
      body: data.map((row, index) => [index + 1, row.name, row.location]),
    });
    doc.save('departments_data.pdf');
    ...
    <div className="tailtable p-4 w-full mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-lg text-gray-800">Departments Management</h2>
          <Link
